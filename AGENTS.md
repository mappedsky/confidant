...
- **Schemas** (`confidant/schema/`): Pydantic v2 models for API response serialization. `CredentialResponse` has both `credential_keys` (always populated — the key names) and `credential_pairs` (only populated when `metadata_only=False` — the decrypted values). Routes pass `include_credential_keys=True` always and `include_credential_pairs` only when the caller has `get` permission and requests `metadata_only=false`.
