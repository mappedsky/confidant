...
def _build_actions(model_class, item, attribute_names):
    ...
    for attr_name in attribute_names:
        if not hasattr(item, attr_name):
            raise ValueError(f"attribute {attr_name} does not exist on model")
        old_value = getattr(item, attr_name)
        if old_value is None:
            continue
        if not isinstance(old_value, bool):
            raise ValueError(
                "attribute {} does not appear to be a boolean "
                "attribute".format(attr_name)
            )

        action = getattr(model_class, attr_name)
        actions.append(action.set(getattr(item, attr_name)))
...
def migrate_boolean_attributes(
    model_class,
    attribute_names,
    ...
    if max_items_updated_per_second <= 0.0:
        msg = "max_items_updated_per_second must be greater than zero"
        raise ValueError(msg)

    read_capacity = read_capacity_to_consume_per_second
    allow_without_rcu = allow_scan_without_rcu

    for item in model_class.rate_limited_scan(
        _build_lba_filter_condition(attribute_names),
        read_capacity_to_consume_per_second=read_capacity,
        page_size=page_size,
        limit=limit,
        allow_rate_limited_scan_without_consumed_capacity=allow_without_rcu,
    ):
        items_processed += 1
        ...
        try:
            ...
            time.sleep(
                max(
                    0,
                    1 / max_items_updated_per_second
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="For development purposes, allow scanning without read capacity units",
)
def migrate_boolean_attribute(
    rcu, page_size, limit, back_off, update_rate, scan_without_rcu
):
    """
    ...
    logger.info(
        ...
        "Max update rate: {}, Attributes: {}".format(
            rcu, page_size, limit, back_off, update_rate, attributes
        )
    )
    model = GenericSecret
